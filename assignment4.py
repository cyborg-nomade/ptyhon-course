"""Assignment 4

1) Write a normal function that accepts another function as an argument.
Output the result of that other function in your “normal” function.

2) Call your “normal” function by passing a lambda function
– which performs any operation of your choice – as an argument.

3) Tweak your normal function by allowing an infinite amount of arguments
on which your lambda function will be executed.

4) Format the output of your “normal” function such that numbers
look nice and are centered in a 20 character column.
"""

import functools

# 1) Write a normal function that accepts another function as an argument.
# Output the result of that other function in your “normal” function.
def normal_func(func_arg, arg1, arg2):
    """prints the result of a function

    Args:
        func_arg (functions): A function
    """
    print(func_arg(arg1, arg2))


# 2) Call your “normal” function by passing a lambda function
# – which performs any operation of your choice – as an argument.
normal_func(lambda arg1, arg2: arg1 + arg2, 1, 2)

# 3) Tweak your normal function by allowing an infinite amount of arguments
# on which your lambda function will be executed.
def normal_func_tweaked(func_arg, *args):
    """prints the result of a function

    Args:
        func_arg (functions): A function
    """
    print(func_arg(*args))


normal_func_tweaked(
    lambda *args: functools.reduce(lambda sum, arg: sum + arg, args, 0), 1, 2, 3
)

# 4) Format the output of your “normal” function such that numbers
# look nice and are centered in a 20 character column.
def normal_func_formatted(func_arg, arg1, arg2):
    """prints the result of a function

    Args:
        func_arg (functions): A function
    """
    print(f"{func_arg(arg1, arg2):_^20}")


normal_func_formatted(lambda arg1, arg2: arg1 + arg2, 1, 2)
